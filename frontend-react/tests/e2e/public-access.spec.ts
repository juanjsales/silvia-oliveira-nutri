...
test('login offers a working password recovery route',async({page})=>{await page.goto('/login');await expect(page.getByRole('heading',{name:'Bem-vindo ao seu espaço'})).toBeVisible();await page.getByRole('link',{name:'Esqueceu sua senha?'}).click();await expect(page).toHaveURL(/recuperar-senha/);await expect(page.getByRole('heading',{name:'Esqueceu sua senha?'})).toBeVisible()});
