...
function Step({ state, selected, busy, act }: { state: OnboardingState; selected: OnboardingStep; busy: boolean; act: (fn: () => Promise<OnboardingState>) => Promise<void> }) {
  ...
  if (selected === 'VERCEL') return <SimpleForm title="Projeto Vercel" description="A conta precisa estar conectada e o nome verificado no passo Vercel." fields={[["projectName", "Nome do projeto", state.vercel.projectName, "text"]]} busy={busy} submit={v => act(() => platformOnboardingApi.save(state.tenantId, 'VERCEL', { projectName: v.projectName }))} />;
  if (selected === 'SUPABASE') return <SimpleForm title="Banco Supabase" description="Informe apenas a referência pública do projeto; nunca cole senhas ou URLs de banco." fields={[["projectRef", "Referência do projeto", state.supabase.projectRef, "text"], ["region", "Região", state.supabase.region, "text"]]} busy={busy} submit={v => act(() => platformOnboardingApi.save(state.tenantId, 'SUPABASE', { projectRef: v.projectRef, region: v.region }))} />;
