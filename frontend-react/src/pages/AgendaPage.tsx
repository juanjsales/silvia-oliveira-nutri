import { CalendarDays, ChevronLeft, ChevronRight, Clock3, ExternalLink, MessageCircle, Plus, UserRound, Video, X } from 'lucide-react';
...
export function AgendaPage() {
  ...
  return <div><div className="page-intro"><div><p>Organize consultas e inicie o atendimento a partir de uma visão única.</p></div><button className="primary-button" onClick={()=>showCreate()}><Plus size={18}/> Nova consulta</button></div>{error&&<div className="form-error" role="alert">{error}</div>}{notice&&<div className="form-success">{notice}</div>}{requests.length>0&&<section className="panel appointment-requests"><div className="panel-heading"><div><span className="eyebrow">Portal do paciente</span><h3>Solicitações pendentes</h3></div><strong>{requests.length}</strong></div><div>{requests.map(item=><article key={item.id}><CalendarDays/><div><strong>{item.patientName}</strong><span>{new Date(`${item.preferredDate}T12:00:00`).toLocaleDateString('pt-BR')} · {{MORNING:'Manhã',AFTERNOON:'Tarde',EVENING:'Noite'}[item.preferredPeriod]} · {item.appointmentType}</span>{item.notes&&<small>{item.notes}</small>}</div><button className="secondary-button" onClick={()=>void declineRequest(item)}>Recusar</button><button className="primary-button" onClick={()=>approveRequest(item)}>Agendar</button></article>)}</div></section>}
    <div className="agenda-layout"><section className="panel calendar-panel"><div className="calendar-heading"><button className="icon-button" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()-1,1))}><ChevronLeft size={19}/></button><h2>{month.toLocaleDateString('pt-BR',{month:'long',year:'numeric'})}</h2><button className="icon-button" onClick={()=>setMonth(new Date(month.getFullYear(),month.getMonth()+1,1))}><ChevronRight size={19}/></button></div><div className="calendar-weekdays">{['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'].map(day=><span key={day}>{day}</span>)}</div><div className="calendar-grid">{cells.map(date=>{const value=isoDate(date);const count=appointments.filter(a=>a.date.slice(0,10)===value).length;return <button key={value} className={`${date.getMonth()!==month.getMonth()?'outside ':''}${value===selected?'selected ':''}${value===todayIso?'today':''}`} onClick={()=>setSelected(value)}><span>{date.getDate()}</span>{count>0&&<small>{count}</small>}</button>})}</div><div className="calendar-legend"><span><i/> Dia com consulta</span><button className="text-link" onClick={()=>{const now=new Date();setMonth(new Date(now.getFullYear(),now.getMonth(),1));setSelected(todayIso)}}>Ir para hoje</button></div></section>
      <section className="panel day-panel"><div className="panel-heading"><div><span className="eyebrow">Agenda do dia</span><h3>{new Date(`${selected}T12:00:00`).toLocaleDateString('pt-BR',{weekday:'long',day:'numeric',month:'long'})}</h3></div><button className="icon-button" onClick={()=>showCreate(selected)} aria-label="Agendar neste dia"><Plus size={20}/></button></div>{loading?<div className="empty-state"><span className="spinner"/><strong>Carregando agenda...</strong></div>:selectedItems.length===0?<div className="empty-state"><CalendarDays size={32}/><strong>Nenhuma consulta neste dia</strong><p>Escolha outro dia ou adicione um novo horário.</p><button className="secondary-button" onClick={()=>showCreate(selected)}><Plus size={17}/> Agendar consulta</button></div>:<div className="appointment-list">{selectedItems.map(item=><article className={`appointment-card response-${item.patientResponse.toLowerCase()}`} key={item.id}><div className="appointment-time"><strong title={`${item.time} às ${getEndTime(item.time, item.durationMinutes)}`}>{item.time} às {getEndTime(item.time, item.durationMinutes)}</strong><small>{item.durationMinutes} min</small></div><div className="appointment-info"><strong>{item.patientName}</strong><span>{item.type}</span><small>{item.patientResponse==='RESCHEDULE_REQUESTED'?`Reagendamento: ${item.patientResponseNote}`:item.patientResponse==='CONFIRMED'?'Paciente confirmou presença':'Aguardando confirmação do paciente'}</small></div><select className={`appointment-status status-${item.status.toLowerCase()}`} value={item.status} onChange={e=>void updateStatus(item,e.target.value as AppointmentStatus)}>{statusTransitions[item.status].map(value=><option key={value} value={value}>{statusLabels[value]}</option>)}</select><div className="appointment-actions">{item.patientResponse==='RESCHEDULE_REQUESTED'&&<button className="secondary-button" onClick={()=>showEdit(item)}>Reagendar</button>}{item.whatsapp&&<a className="icon-button" href={getWhatsAppLink(item)!} target="_blank" rel="noreferrer" title="Enviar lembrete e link da videochamada no WhatsApp" style={{color:'#16a34a'}}><MessageCircle size={17}/></a>}{item.meetingUrl&&<a className="icon-button" href={item.meetingUrl} target="_blank" rel="noreferrer" title="Abrir teleconsulta"><Video size={17}/></a>}<Link className="start-care" to={`/atendimentos?paciente=${item.patientId}&agendamento=${item.id}${item.type.toLowerCase().includes('online')?'&video=true':''}`}>Atender <ChevronRight size={16}/></Link></div></article>)}</div>}</section></div>
