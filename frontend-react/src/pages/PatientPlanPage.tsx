...
type Plan={id:string;title:string;objective?:string;status:'PUBLISHED'|'ARCHIVED';publishedAt?:string;updatedAt:string;content:{meals?:any[];refeicoes?:any[];patientVisibility?:'FULL'|'SUMMARY'|'HIDDEN'}};
...
export function PatientPlanPage(){
 ...
 const meals=plan.content.meals||plan.content.refeicoes||[],showNumbers=plan.content.patientVisibility!=='HIDDEN';
 return <main className="patient-plan"><header><Link to="/portal"><ArrowLeft/> Voltar ao portal</Link><button onClick={()=>window.print()}><Printer/> Imprimir</button></header><article>
  ...
  {meals.map((meal,index)=><section key={index}><header><span>{showNumbers?(meal.time||meal.horario||'Horário flexível'):'Horário conforme orientação'}</span><h2>{meal.title||meal.titulo||'Refeição'}</h2></header>{(meal.items||meal.alimentosList||[]).map((item:any,i:number)=><div key={i}><strong>{item.name||item.nome}</strong><span>{showNumbers?(item.amountText||((item.amount??item.qtd)?`${item.amount??item.qtd} ${item.unit||item.unidade||'g'}`:'Conforme orientação')):'Conforme orientação'}</span></div>)}{meal.substitutions?.length?<div className="patient-substitutions"><strong>Substituições</strong><ul>{meal.substitutions.map((item:string|{option?:string;equivalence?:string},i:number)=><li key={i}>{typeof item==='string'?item:<><b>{item.option}</b>{item.equivalence&&<small> · {item.equivalence}</small>}</>}</li>)}</ul></div>:null}{(meal.notes||meal.obs)&&<p>{meal.notes||meal.obs}</p>}</section>)}
 </article></main>
}
