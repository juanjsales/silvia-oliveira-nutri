...
type AuthContextValue = { user: AuthUser | null; loading: boolean; login: (identifier: string, password: string) => Promise<void>; logout: () => Promise<void> };
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({ user, loading, login: async (identifier, password) => { const result = await api<{ user: AuthUser }>('/api/auth/login', { method: 'POST', body: JSON.stringify({ identifier, password }) }); setUser(result.user); }, logout: async () => { await api('/api/auth/logout', { method: 'POST' }); setUser(null); } }), [user, loading]);
