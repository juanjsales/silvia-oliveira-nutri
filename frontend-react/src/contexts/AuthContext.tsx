...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { api<{ user: AuthUser & { userId?: string } }>('/api/auth/me').then(({ user: current }) => { const normalized={ ...current, id: current.id ?? current.userId! }; setUser(normalized); if(normalized.role==='PATIENT') void import('../pages/PatientPortalPage'); }).catch((error) => { if (!(error instanceof ApiError) || error.status !== 401) console.error(error); }).finally(() => setLoading(false)); }, []);
  const value = useMemo<AuthContextValue>(() => ({ user, loading, login: async (identifier, password) => { const result = await api<{ user: AuthUser }>('/api/auth/login', { method: 'POST', body: JSON.stringify({ identifier, password }) }); setUser(result.user); if(result.user.role==='PATIENT') void import('../pages/PatientPortalPage'); return result.user; }, logout: async () => { setUser(null); try { await api('/api/auth/logout', { method: 'POST' }); } catch { /* A saída local não depende da disponibilidade do servidor. */ } } }), [user, loading]);
