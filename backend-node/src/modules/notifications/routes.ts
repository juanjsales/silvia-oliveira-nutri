...
export async function notificationRoutes(app: FastifyInstance) {
  ...
  app.get('/', async request => {
    ...
    const reconcile=async(label:string,sql:string)=>{try{await app.db.query(sql)}catch(error){app.log.warn({err:error,reconciliation:label},'Falha parcial ao reconciliar notificações')}};
    await reconcile('appointment-requests',`INSERT INTO professional_notifications(kind,title,body,priority,entity_type,entity_id,action_url,dedupe_key)
    ...
    await reconcile('messages',`INSERT INTO professional_notifications(kind,title,body,priority,entity_type,entity_id,action_url,dedupe_key)
    ...
    await reconcile('finance-resolution',`UPDATE professional_notifications SET status='RESOLVED',resolved_at=COALESCE(resolved_at,now()),updated_at=now() WHERE dedupe_key='finance:overdue' AND status='ACTIVE' AND NOT EXISTS(SELECT 1 FROM financial_transactions WHERE status='OVERDUE')`);
    await reconcile('checkins',`INSERT INTO professional_notifications(kind,title,body,priority,entity_type,entity_id,action_url,dedupe_key)
      ...
      FROM preconsult_checkins c JOIN patients p ON p.id=c.patient_id JOIN appointments a ON a.id=c.appointment_id
      WHERE c.status='PENDING_REVIEW' AND a.status IN('CONFIRMED','WAITING','IN_PROGRESS')
      ON CONFLICT(dedupe_key) DO UPDATE SET status=CASE WHEN professional_notifications.status='ARCHIVED' THEN 'ARCHIVED' ELSE 'ACTIVE' END,updated_at=now()`);
    await reconcile('exams',`INSERT INTO professional_notifications(kind,title,body,priority,entity_type,entity_id,action_url,dedupe_key)
      ...
      ON CONFLICT(dedupe_key) DO UPDATE SET status=CASE WHEN professional_notifications.status='ARCHIVED' THEN 'ARCHIVED' ELSE 'ACTIVE' END,updated_at=now()`);
    await reconcile('resolved-items',`UPDATE professional_notifications n SET status='RESOLVED',resolved_at=COALESCE(n.resolved_at,now()),updated_at=now()
      WHERE n.status='ACTIVE' AND ((n.entity_type='appointment_request' AND NOT EXISTS(SELECT 1 FROM appointment_requests r WHERE r.id=n.entity_id AND r.status='PENDING')) OR (n.entity_type='preconsult_checkin' AND NOT EXISTS(SELECT 1 FROM preconsult_checkins c JOIN appointments a ON a.id=c.appointment_id WHERE c.id=n.entity_id AND c.status='PENDING_REVIEW' AND a.status IN('CONFIRMED','WAITING','IN_PROGRESS'))) OR (n.entity_type='patient_exam' AND NOT EXISTS(SELECT 1 FROM patient_exam_uploads e WHERE e.id=n.entity_id AND e.status='SENT')) OR (n.entity_type='patient_message' AND NOT EXISTS(SELECT 1 FROM patient_messages m WHERE m.id=n.entity_id AND m.read_at IS NULL)) OR (n.entity_type='privacy_request' AND NOT EXISTS(SELECT 1 FROM data_subject_requests r WHERE r.id=n.entity_id AND r.status IN('OPEN','IN_REVIEW'))))`);
