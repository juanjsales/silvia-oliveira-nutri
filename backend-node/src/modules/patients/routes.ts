...
export async function patientRoutes(app: FastifyInstance) {
  ...
  app.post('/:id/access', async (request, reply) => {
    ...
    let emailSent=false;try{emailSent=await sendPatientAccessEmail(app.env,{to:row.email,name:row.name,temporaryPassword:password})}catch(error){app.log.error({err:error,patientId:id},'Falha ao enviar acesso do paciente')}
