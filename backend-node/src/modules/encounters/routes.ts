...
export async function encounterRoutes(app: FastifyInstance) {
  ...
  ]).parse(request.body);const client=await app.db.connect();try{await client.query('BEGIN');const encounter=(await client.query<{patientId:string;status:string;correctionOpen:boolean}>(`SELECT patient_id AS "patientId",status,correction_open AS "correctionOpen" FROM clinical_encounters WHERE id=$1 FOR UPDATE`,[id])).rows[0];if(!encounter){await client.query('ROLLBACK');return reply.code(404).send({error:'Atendimento não encontrado.'})}if(encounter.status!=='IN_PROGRESS'&&!(encounter.status==='COMPLETED'&&encounter.correctionOpen)){await client.query('ROLLBACK');return reply.code(409).send({error:'Abra uma correção antes de alterar um prontuário finalizado.'})}
