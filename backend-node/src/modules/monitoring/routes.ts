...
export async function monitoringRoutes(app:FastifyInstance) {
  ...
  app.post('/incidents/resolve-all',async(request)=>{const result=await app.db.query(`UPDATE system_incidents SET resolved_at=now(),resolved_by=$1 WHERE resolved_at IS NULL RETURNING id`,[request.auth!.userId]);await audit(app.db,'SYSTEM_ALL_INCIDENTS_RESOLVED','system_incident',{actorUserId:request.auth!.userId,metadata:{count:result.rowCount}});return{message:`${result.rowCount||0} incidentes marcados como resolvidos.`}});
}
